...
The latest stable release is: [1.3.3][main-release].
...
The latest development release is: <ins>1.3.4-SNAPSHOT</ins>.
```bash
wget -O opentracing-specialagent-1.3.4-SNAPSHOT.jar "https://oss.sonatype.org/service/local/artifact/maven/redirect?r=snapshots&g=io.opentracing.contrib.specialagent&a=opentracing-specialagent&v=LATEST"
```
...
1. The `default` profile is used for development of [<ins>Instrumentation Rules</ins>](#45-instrumentation-rule). It builds and runs tests for each rule, but _does not bundle the rules_ into [`opentracing-specialagent-1.3.3.jar`][main-release]
...
1. The `assemble` profile is used to bundle the [<ins>Instrumentation Rules</ins>](#45-instrumentation-rule) into [`opentracing-specialagent-1.3.3.jar`][main-release]. It builds each rule, but _does not run tests._ Once the build with the `assemble` profile is finished, the [`opentracing-specialagent-1.3.3.jar`][main-release] will contain the built rules inside it.

    _**Note**: If you do not run this step, the [`opentracing-specialagent-1.3.3.jar`][main-release] from the previous step will not contain any [<ins>Instrumentation Plugins</ins>](#44-instrumentation-plugin) within it!_
...
```bash
java -javaagent:opentracing-specialagent-1.3.3.jar -jar MyApp.jar
```
...
1. For jdk1.8
    ```bash
    java -Xbootclasspath/a:$JAVA_HOME/lib/tools.jar -jar opentracing-specialagent-1.3.3.jar <PID>
    ```

1. For jdk9+
    ```bash
    java -jar opentracing-specialagent-1.3.3.jar <PID>
    ```
...
```bash
java -javaagent:opentracing-specialagent-1.3.3.jar -Dsa.spring -jar MySpringApp.jar
```
...
[main-release]: http://central.maven.org/maven2/io/opentracing/contrib/specialagent/opentracing-specialagent/1.3.3/opentracing-specialagent-1.3.3.jar
[main-snapshot]: https://oss.sonatype.org/content/repositories/snapshots/io/opentracing/contrib/specialagent/opentracing-specialagent/1.3.4-SNAPSHOT
...
[test-release]: http://central.maven.org/maven2/io/opentracing/contrib/specialagent/opentracing-specialagent/1.3.3/opentracing-specialagent-1.3.3-tests.jar
[test-snapshot]: https://oss.sonatype.org/content/repositories/snapshots/io/opentracing/contrib/specialagent/opentracing-specialagent/1.3.4-SNAPSHOT