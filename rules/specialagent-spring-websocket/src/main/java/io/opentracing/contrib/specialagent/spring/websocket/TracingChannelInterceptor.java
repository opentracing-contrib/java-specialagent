...
public class TracingChannelInterceptor implements  ExecutorChannelInterceptor {
  ...
  private Message<?> preSendServerSpan(Message<?> message) {
    final SpanBuilder spanBuilder = tracer.buildSpan((String) message.getHeaders()
        .getOrDefault(SIMP_DESTINATION, UNKNOWN_DESTINATION))
        .withTag(Tags.SPAN_KIND.getKey(), spanKind)
        .withTag(Tags.COMPONENT.getKey(), WEBSOCKET);

    Map<String, List<String>> nativeHeaders = (Map<String, List<String>>) message.getHeaders().get(
        NativeMessageHeaderAccessor.NATIVE_HEADERS);
    SpanContext spanContext = null;
    if (nativeHeaders != null) {
      spanContext = tracer
          .extract(Builtin.TEXT_MAP, new NativeHeadersExtractAdapter(nativeHeaders));
    }
    if (spanContext == null) {
      spanContext = tracer
