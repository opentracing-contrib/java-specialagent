...
public class KafkaConsumerAgentRule implements AgentRule {
  @Override
    ...
    return Arrays.asList(new AgentBuilder.Default().
        ...
        .transform(new Transformer() {
          ...
          public Builder<?> transform(final Builder<?> builder,
            ...
            return builder
                .visit(Advice.to(KafkaConsumerAgentRule.class)
                    .on(named("poll").and(takesArguments(1))));
          }
        }));
  }

  @Advice.OnMethodExit
  public static void exit(@Advice.Return(typing = Typing.DYNAMIC) Object returned) {
    if (AgentRuleUtil.isEnabled()) {
      KafkaAgentIntercept.consumerRecords(returned);
    }
  }
