...
public class Jedis2AgentRule extends AgentRule {
  ...
  public Iterable<? extends AgentBuilder> buildAgent(final String agentArgs,
    ...
    return Arrays.asList(narrowable.transform(new Transformer() {
      ...
      public Builder<?> transform(final Builder<?> builder, final TypeDescription typeDescription,
        ...
        return builder.visit(Advice.to(SendCommand.class)
            .on(named("sendCommand").and(takesArgument(1, byte[][].class))));
      }
    }), narrowable.transform(new Transformer() {
      @Override
      public Builder<?> transform(final Builder<?> builder, final TypeDescription typeDescription,
          final ClassLoader classLoader, final JavaModule module) {
        return builder.visit(Advice.to(ReadCommandOutput.class)
            .on(named("readProtocolWithCheckingBroken")));
      }
    }), narrowable.transform(new Transformer() {
      @Override
      public Builder<?> transform(final Builder<?> builder, final TypeDescription typeDescription,
          final ClassLoader classLoader, final JavaModule module) {
        return builder.visit(Advice.to(OnError.class)
            .on(named("readProtocolWithCheckingBroken")));
      }
    }));
  }
  ...
  public static class SendCommand {
    ...
    public static void enter(final @Advice.Origin String origin,
        final @Advice.Argument(value = 0, typing = Typing.DYNAMIC) Object command,
        @Advice.Argument(value = 1, readOnly = false, typing = Typing.DYNAMIC) byte[][] args) {
