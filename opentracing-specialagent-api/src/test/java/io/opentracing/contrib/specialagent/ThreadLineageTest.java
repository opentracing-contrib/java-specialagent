...
public class ThreadLineageTest {
  ...
  private static void startTracerThreads(final CountDownLatch latch) throws Exception {
    final Class<?> cls = AgentRuleUtil.tracerClassLoader.loadClass("io.opentracing.contrib.specialagent.TestTracer");
    assertEquals("Tracer class should be loaded from tracer class loader", AgentRuleUtil.tracerClassLoader, cls.getClassLoader());
  ...
  public void test() throws Exception {
    for (int i = 0; i < systemClasses.length; ++i)
      assertEquals("System class should be loaded from system class loader", systemClasses[i], AgentRuleUtil.tracerClassLoader.loadClass(systemClasses[i].getName()));
