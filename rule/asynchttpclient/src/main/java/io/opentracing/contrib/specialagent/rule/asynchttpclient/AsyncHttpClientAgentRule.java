...
public class AsyncHttpClientAgentRule extends AgentRule {
  ...
  public AgentBuilder buildAgentChainedGlobal1(final AgentBuilder builder) {
    return builder
      .type(not(isInterface()).and(hasSuperType(named("org.asynchttpclient.AsyncHttpClient"))))
      .transform(new Transformer() {
        @Override
        public Builder<?> transform(final Builder<?> builder, final TypeDescription typeDescription, final ClassLoader classLoader, final JavaModule module) {
          return builder.visit(advice(typeDescription).to(AsyncHttpClientAgentRule.class).on(named("executeRequest").and(takesArguments(2)).and(takesArgument(0, named("org.asynchttpclient.Request")))));
        }});
  }
